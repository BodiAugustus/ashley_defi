// This file  is the subscribe/members oage
...
export default function Marketplace({ courses }) {
  ...
  const purchaseCourse = async (order, course) => {
    //passes order into Modal component
    // console.log(order);
    // console.log(course);
      ...
      <div className="relative bg-black overflow-hidden">
        <div className="relative max-w-7xl mx-auto ">
          <div className="fit pb-4">
            ...
            <Terms />
            {/* Once we have a selectedCourse then the modal opens up with that courses data */}
            {selectedCourse && (
              <OrderModal
...
export function getStaticProps() {
  // to fetch data
  const { data } = getAllCourses(); // Gets the data from the JSON file
  return {
    props: {
