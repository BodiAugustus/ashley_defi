...
const OrderModal = ({course, onClose, onSubmit, isNewPurchase}) => {     //When orderModal receives course data then we open the modal with that course info
    ...
    useEffect(() => {
        if(!!course){ //double !! will result a null or undefined returned value into false to avoid throwing an error
            ...
            setOrder({
              ...defaultOrder,
              price: eth.perItem //sets new price on default order
            })
        }
    },[course])
